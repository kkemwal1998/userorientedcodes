...
# In this, we will be using __str__ in order to deal with the string as our object in the function under the class. This will not make a such diffurece in our program. It is just an alternative. 
...
class Student: # In this, we will be using the keyword called class under which Student will act as a cintainer for costum names.
    ...
    def __str__(self) :        
        return f"{self.name} from {self.house}"    # Thus, we will be using f string for the same.
